...
Prefer simple, explicit solutions. Follow the project’s tech stack: Next.js 15
App Router for Chapter 6+ deliverables (legacy surfaces remain on the existing
Vite frontend), Vitest/Playwright for testing, ESLint + Prettier for
linting/formatting. New tools require a justification and approval via Governance.
_Updated per Chapter 6 Workbook requirements._

## Project Standards & Constraints
- Frontend lives in `frontend/` and is built with Next.js 15 App Router for
  Chapter 6+ deliverables (legacy surfaces remain on the existing Vite build).
  The final build in `frontend/dist/` MUST be deployable to GitHub Pages.
