...
- [X] T007 [P] Create `api/src/schemas/index.ts` with Zod validation schemas:
...
- [X] T008 [P] Create `api/src/middleware/validation.ts` - enhance existing or create `validateBody` middleware:
...
- [X] T009 Update `/api/spec/openapi.yaml` with POST `/expenses` endpoint:
...
- [X] T010 Update `/api/spec/openapi.yaml` with GET `/expenses/summary` endpoint:
...
- [X] T011 Update `/api/spec/openapi.yaml` with complete error response schema for both endpoints
...
- [X] T012 [P] Create `api/src/services/expenses.ts` with `ExpenseStore` class:
...
- [X] T013 [P] Create `api/src/services/expenses.ts` continuation - `ExpenseStore` aggregation:
...
- [X] T014 [P] Create singleton export in `api/src/services/expenses.ts`:
...
- [X] T015 Implement POST `/expenses` route handler in `api/src/routes/expenses.ts`:
...
- [X] T016 Implement GET `/expenses/summary` route handler in `api/src/routes/expenses.ts`:
...
- [X] T017 Register routes in `api/src/server.ts`:
...
- [X] T018 [P] Create `api/tests/integration/expenses.test.ts` - POST /expenses success cases (Supertest):
...
- [X] T019 [P] Create `api/tests/integration/expenses.test.ts` - POST /expenses validation error cases:
...
- [X] T020 [P] Create `api/tests/integration/expenses.test.ts` - GET /expenses/summary no filters:
...
- [X] T021 [P] Create `api/tests/integration/expenses.test.ts` - GET /expenses/summary with category filter:
...
- [X] T022 [P] Create `api/tests/integration/expenses.test.ts` - GET /expenses/summary with month filter:
...
- [X] T023 [P] Create `api/tests/integration/expenses.test.ts` - GET /expenses/summary with both filters:
...
- [X] T024 [P] Create `api/tests/unit/expenses.test.ts` - ExpenseStore.create():
...
- [X] T025 [P] Create `api/tests/unit/expenses.test.ts` - ExpenseStore.filter():
...
- [X] T026 [P] Create `api/tests/unit/expenses.test.ts` - ExpenseStore.summarize():
...
- [X] T027 [P] Run `npm run test:coverage` in `api/` directory and verify:
...
- [X] T028 Update `api/README.md` or create endpoint documentation:
  - Link to OpenAPI spec for `/expenses` and `/expenses/summary`
  - Quick reference for curl examples from quickstart.md
  - Environment setup instructions

- [X] T029 Execute quickstart.md acceptance scenarios manually:
...
- [X] T030 Verify no console errors or warnings:
...
- [X] T031 [POLISH] Add JSDoc comments to all exported functions in service/routes
- [X] T032 [POLISH] Add error handler middleware for 500 errors (if not present)
- [X] T033 [POLISH] Validate OpenAPI spec with `npm run validate:openapi` (if available)
- [X] T034 [POLISH] Create Postman collection from OpenAPI spec for team sharing
- [X] T035 [POLISH] Add request/response logging via Pino logger
