...
- [X] **T001** Audit current `.github/workflows/test.yml` to understand existing coverage configuration and identify gaps in coverage generation/copying/verification
- [X] **T002** Verify vitest.config.js has coverage enabled with reporters: ["lcov", "html"] and all five UI test suites are included or covered by pattern matching
- [X] **T003** Create `.github/scripts/` directory if it doesn't exist to house helper scripts for coverage copying and verification
...
- [X] **T004** [P] Write contract test for Coverage Generation in `tests/contracts/test-coverage-generation.js` - verify Vitest executes all five UI test suites and produces lcov.info + HTML for each (MUST FAIL - no implementation yet)
- [X] **T005** [P] Write contract test for Coverage Copying in `tests/contracts/test-coverage-copying.js` - verify all five coverage directories are successfully copied from coverage/ to review-artifacts/ (MUST FAIL)
- [X] **T006** [P] Write contract test for Coverage Verification in `tests/contracts/test-coverage-verification.js` - verify verification script confirms all five suites have lcov.info and index.html files with minimum size requirements (MUST FAIL)
- [X] **T007** [P] Write contract test for Artifact Upload in `tests/contracts/test-artifact-upload.js` - verify artifact packaging only proceeds if verification passes (MUST FAIL)
- [X] **T008** [P] Integration test: End-to-end workflow in `tests/integration/test-review-packet-workflow.js` - simulate complete flow from coverage generation → copying → verification → artifact (MUST FAIL)
...
- [X] **T009** Update `vitest.config.js` to ensure coverage is enabled with reporters: ["lcov", "html", "text-summary"] for all five UI test suites (ui-expense, ui-stopwatch, ui-temp, ui-todo, ui-quote)
- [X] **T010** Set coverage output directory to `coverage/` in vitest.config.js with 40% minimum statement threshold per Constitutional requirement
- [X] **T011** Verify each test suite generates output in separate coverage/[suite-name]/ directory structure (not aggregated)

### CI Workflow: Coverage Generation Step

- [X] **T012** Audit existing test.yml workflow to see if coverage is already being generated; if not, add step: `npm run test:ui:coverage` with fail-fast behavior (exit code check)
- [X] **T013** Create npm script `test:ui:coverage` in package.json that runs Vitest with coverage enabled for all five UI test suites: `vitest run --coverage -- frontend/tests/ui-*.test.js`
- [X] **T014** Ensure coverage generation step fails the build immediately if any test suite fails or coverage is below 40% threshold

### Coverage Copying Script

- [X] **T015** Create `.github/scripts/copy-coverage-reports.sh` bash script that:
...
- [X] **T016** Add copy script step to `.github/workflows/test.yml` workflow: `bash .github/scripts/copy-coverage-reports.sh` after coverage generation completes

### Coverage Verification Script

- [X] **T017** Create `.github/scripts/verify-coverage-reports.sh` bash script that:
...
- [X] **T018** Add verification step to `.github/workflows/test.yml` workflow: `bash .github/scripts/verify-coverage-reports.sh` after coverage copying completes
...
- [X] **T019** Audit `review-artifacts/index.html` to ensure it references all five coverage suites with correct relative links:
...
- [X] **T020** Update review-artifacts/index.html if needed to include all five suite links with proper formatting

### CI Workflow Artifact Upload

- [X] **T021** Add artifact upload step to `.github/workflows/test.yml` using `actions/upload-artifact@v4`:
  - Runs only if verification step succeeds (`if: success()`)
  - Artifact name: `review-packet`
  - Artifact path: `review-artifacts/`
  - Retention days: 90

- [X] **T022** Ensure workflow fails immediately on any error (fail-fast behavior):
...
- [X] **T023** Verify complete `.github/workflows/test.yml` workflow contains steps in correct order:
...
- [X] **T024** [P] Run quickstart.md Step 1: Verify Vitest configuration - confirm all five suites have coverage enabled
- [X] **T025** [P] Run quickstart.md Step 2: Generate local coverage with `npm run test:ui:coverage` - verify all five suites execute
- [X] **T026** [P] Run quickstart.md Step 3: Verify coverage files exist - ensure lcov.info and index.html for all five suites in coverage/
- [X] **T027** [P] Run quickstart.md Step 4: Execute copy script locally - verify files copied to review-artifacts/
- [X] **T028** [P] Run quickstart.md Step 5: Execute verification script locally - confirm all reports verified successfully

### Failure Scenario Testing

- [X] **T029** Test failure scenario: Remove one coverage file and run verification script - confirm it fails with proper error message
- [X] **T030** Test failure scenario: Mock test failure by temporarily modifying a test file - confirm coverage generation fails and verification is skipped
- [X] **T031** Test failure scenario: Delete a suite directory - confirm verification fails and artifact upload is prevented

### Documentation & Cleanup

- [X] **T032** Create/update docs/COVERAGE_WORKFLOW.md documenting:
...
- [X] **T033** Add comments to `.github/scripts/copy-coverage-reports.sh` explaining the coverage copying process
- [X] **T034** Add comments to `.github/scripts/verify-coverage-reports.sh` explaining the verification logic

### Final Integration Test

- [X] **T035** Run quickstart.md Step 8: Simulate complete CI workflow locally - execute all steps in sequence and confirm success
- [X] **T036** Run quickstart.md Step 9: Verify CI workflow configuration - check .github/workflows/test.yml has all required steps
- [X] **T037** Commit all changes with message: "feat(017): Fix review-packet packaging for all five UI test suites"
